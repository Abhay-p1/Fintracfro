...
app.post("/consent/:mobileNumber", (req, res) => {
  ...
  if (token) 
    jwt.verify(token,config.JWT_secret, (err, decoded)=> {
    if (!err) res.send({ url:"http://localhost:5500", token: token });
    })
  obj={
  ...
  .then(()=>{
        ...
        .then(resp=>{
          ...
          if(resp)
            res.send({ url:"http://localhost:5500/index.html", token: tokenjwt }); 
          else
            res.send({ url:"http://localhost:5500/index.html", token: null });
        return
        }).catch(()=>{
